              ...
              {step === 'success' && (
                <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center py-16 text-center">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    <motion.button onClick={() => router.push('/')} className="inline-flex items-center justify-center gap-2 bg-primary text-white px-6 py-3 rounded-full font-semibold text-sm" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                      Volver al Inicio
                    </motion.button>
                    <motion.a href={`https://wa.me/${whatsappNum}?text=${encodeURIComponent('Hola iTech Peru! Acabo de realizar un pago')}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 bg-[#25D366] text-white px-6 py-3 rounded-full font-semibold text-sm" whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
