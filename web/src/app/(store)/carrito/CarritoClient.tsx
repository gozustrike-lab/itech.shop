...
export default function CarritoClient() {
  ...
  const handleWhatsApp = useCallback(() => {
    ...
    const message = encodeURIComponent(
      `Hola ${siteConfig.title || 'iTech Peru'}! Quisiera hacer el siguiente pedido:\n\n${items}\n\nTotal: ${formatPrice(cartTotal)}`
    );
    window.open(`https://wa.me/${whatsappNum}?text=${message}`, '_blank');
  }, [cart, cartTotal, siteConfig.title, whatsappNum]);
        ...
        {cart.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-foreground/30 text-sm mb-6">
              Agrega los equipos o accesorios que te gusten y realiza tu pedido
            </p>
            <motion.button
