...
export default function SearchClient({ initialProducts, initialCategories }: SearchClientProps = {}) {
  ...
  const categoryList = useMemo(() => {
    if (Array.isArray(initialCategories) && initialCategories.length > 0) {
      return [
        { id: 'todos', label: 'Todos', count: productList.length },
        ...initialCategories.map((c: any) => ({
          id: c.slug,
          label: c.name,
          count: productList.filter((p: any) => p.category === c.slug).length,
        })),
      ];
    }
    return categories;
  }, [initialCategories, productList]);
          ...
          <div className="relative flex items-center gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Buscar por equipo, marca o modelo... (iPhone, MacBook, PlayStation, Sony)"
            ...
            <button
              ...
              className={`relative w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${
                showFilters || hasActiveFilters
                  ...
                  : 'bg-zinc-100 text-foreground/60 hover:bg-zinc-200'
              }`}
              aria-label="Filtros"
            >
              <SlidersHorizontal className="w-4 h-4" />
              {hasActiveFilters && (
                <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-turquoise-400 border-2 border-background" />
              )}
            </button>
          </div>
